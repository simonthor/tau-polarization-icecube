...
int main(int argc, char **argv) {
  ...
  while (!input_file.failed()) {
    ...
    TauolaHEPEVTEvent* hepevt = new TauolaHEPEVTEvent();

    // Iterate over all particles in the event, find the tau particle (abs(pdg_id) == 15) and decay it using decayOne
    for (auto p : evt.particles()) {
      if (abs(p->pid()) == 15) {
        // TauolaHepMC3Particle *htau_hepmc = new TauolaHepMC3Particle(p);
        // TauolaHEPEVTParticle *nucleus = new TauolaHEPEVTParticle(
        //   100, 1, 
        //   p->momentum().x(), 
        //   p->momentum().y(),  
        //   p->momentum().z(),
        ...
        // TauolaHEPEVTParticle *htau = new TauolaHEPEVTParticle(
        //   p->pid(), 1, 
        //   p->momentum().x(), 
        //   p->momentum().y(),  
        //   p->momentum().z(),
        ...
        // std::cout << "HEPEVTParticle created" << std::endl;
        hepevt->addParticle(htau);
        // std::cout << "HEPEVTParticle added to event\n";
