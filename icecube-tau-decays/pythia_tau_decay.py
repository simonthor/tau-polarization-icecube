...
def main():
    ...
    taus = particle_info[particle_info["pdg"] == tau_pdg_id]
    pythia_decays = []
    ...
    for tau in taus.itertuples():
        ...
        for i, particle in enumerate(event_particles):
            if not particle.isFinal():
                continue
            # print(f"Particle {i}: PDG ID = {particle.id()} 4-momentum = {particle.px()}, {particle.py()}, {particle.pz()}, {particle.e()} (GeV)")
            pythia_decays.append([
                tau.event_num,
                particle.e(),
                particle.id(),
                particle.px(),
    ...
    pythia_decays_df = pd.DataFrame(pythia_decays, columns=["event_num", "energy", "pdg_id", "px", "py", "pz"])
